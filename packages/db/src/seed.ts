...
async function main() {
  ...
  if (existingByEmail) {
    console.log(`Admin user ${adminEmail} already exists, skipping admin seed.`);
  } else if (existingSuperadmin) {
    console.log(
      `Superadmin already exists (${existingSuperadmin.email}); skipping default admin@localhost seed.`,
    );
  } else {
  ...
  // Default firewall rules (idempotent — skip if any inbound rules exist)
  const firewallCount = await prisma.firewallRule.count();
  if (firewallCount === 0) {
    ...
    console.log("Seeded default firewall rules.");
  }
}
